describe('Upload', () => {
    ...
    it('Upload single receipt!', () => {
        ...
        cy.get('#operator-invoice-code-INV-2774').click();
        cy.wait(3000);
        ...
        cy.get('#invoiceno').invoke('val').then(text => { const invoice_no = text;
          ...
          cy.get('#canvas-form-gross-amount').invoke('val').then(text => { const gross_amount1 = text;
              ...
              const tax_amount= tax_amount1.replace(/,/g,"");
                  ...
                  cy.writeFile('AzureAI_Multiple_reciept_17/sep.csv','Invoice details web url '+','+ 'Invoice Number'+','+'VAT no'+ ','+
          ...
          cy.get('#invoice-items-table').each(($row, index, $rows) => {
              ...
              cy.writeFile('AzureAI_Multiple_MultiLine_reciept.csv',itemNo+','+startDate+','+endDate+','+budgetGroup+','+category+','+subCategory+','+description+
      ...
      cy.go('back');
        cy.get('#invoices_list_refresh').click({force: true});
        cy.get('#operator-invoice-code-INV-2026').click();
        cy.wait(3000);
      ...
      cy.go('back');
        cy.get('#invoices_list_refresh').click({force: true});
        cy.get('#operator-invoice-code-INV-2027').click();
        cy.wait(3000);
